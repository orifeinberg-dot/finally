...
class TestPriceCache:
    ...
    def test_mixed_case_updates_hit_same_entry(self):
        """Test that differently-cased tickers all resolve to one entry."""
        cache = PriceCache()
        cache.update("aapl", 190.00)
        cache.update("AAPL", 191.00)
    ...
    def test_get_history_returns_updates_oldest_first(self):
        """Test that get_history() returns updates in chronological order."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)
        cache.update("AAPL", 191.00)
        cache.update("AAPL", 192.00)
    ...
    def test_get_history_unknown_ticker(self):
        """Test that get_history() returns an empty list for unknown tickers."""
        cache = PriceCache()
        assert cache.get_history("NOPE") == []

    def test_get_history_capped_at_60(self):
        """Test that history retains at most the last 60 updates."""
        cache = PriceCache()
        for i in range(100):
            cache.update("AAPL", 100.0 + i)
        history = cache.get_history("AAPL", n=100)
        assert len(history) == 60
        # Should hold the most recent 60 updates (prices 40..99 -> 140..199)
        assert history[0].price == 140.0
        assert history[-1].price == 199.0

    def test_get_history_with_n(self):
        """Test that get_history(..., n=...) limits the number of results."""
        cache = PriceCache()
        for i in range(10):
            cache.update("AAPL", 100.0 + i)
    ...
    def test_get_history_cleared_on_remove(self):
